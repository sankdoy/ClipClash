...
var chatCooldownMs = 800;
var voteCooldownMs = 400;
var reportCooldownMs = 3e3;
...
var updateNameSchema = external_exports.object({
  type: external_exports.literal("update_name"),
  name: external_exports.string().min(1).max(24)
});
var chatSchema = external_exports.object({
  type: external_exports.literal("chat"),
  message: external_exports.string().min(1).max(240)
});
var setTimerSchema = external_exports.object({
  type: external_exports.literal("set_timer"),
  minutes: external_exports.number().int().min(1)
});
var startHuntSchema = external_exports.object({ type: external_exports.literal("start_hunt") });
var resetMatchSchema = external_exports.object({ type: external_exports.literal("reset_match") });
var updateCategoriesSchema = external_exports.object({
  type: external_exports.literal("update_categories"),
  categories: external_exports.array(external_exports.object({ id: external_exports.string().optional(), name: external_exports.string() }))
});
var saveDraftSchema = external_exports.object({
  type: external_exports.literal("save_draft"),
  categoryId: external_exports.string().min(1),
  url: external_exports.string().max(400)
});
var submitSubmissionSchema = external_exports.object({
  type: external_exports.literal("submit_submission"),
  categoryId: external_exports.string().min(1),
  url: external_exports.string().min(1).max(400)
});
var voteSubmissionSchema = external_exports.object({
  type: external_exports.literal("vote_submission"),
  entryId: external_exports.string().min(1)
});
var rpsSchema = external_exports.object({
  type: external_exports.literal("rps_choice"),
  choice: external_exports.enum(["rock", "paper", "scissors"])
});
var reportSchema = external_exports.object({
  type: external_exports.literal("report"),
  messageId: external_exports.string().min(1)
});
var setAudienceModeSchema = external_exports.object({
  type: external_exports.literal("set_audience_mode"),
...
__name(toServerMessage, "toServerMessage");
...
function resolveSessionToken(tokenToPlayerId, token, createPlayerId, createToken) {
...
__name(resolveSessionToken, "resolveSessionToken");
function replaceSocketForToken(tokenToSocket, sessionToken, socket) {
...
__name(replaceSocketForToken, "replaceSocketForToken");
function selectHost(currentHostId, candidateId) {
  return currentHostId ?? candidateId;
}
__name(selectHost, "selectHost");
var src_default = {
...
var RoomsDO = class {
  ...
  constructor(state, env) {
    ...
    this.votesByPlayer = /* @__PURE__ */ new Map();
    this.submissions = /* @__PURE__ */ new Map();
    this.draftsByPlayer = /* @__PURE__ */ new Map();
    this.tiebreak = null;
    this.scoreboard = /* @__PURE__ */ new Map();
    this.history = [];
    this.reports = [];
    this.hostId = null;
    this.tokenToPlayerId = /* @__PURE__ */ new Map();
    this.tokenToSocket = /* @__PURE__ */ new Map();
    this.playerAccounts = /* @__PURE__ */ new Map();
  ...
  async webSocketMessage(ws, message) {
    ...
    if (parsed.type === "hello") {
      ...
      if (existingRecord) {
        ...
        this.players.set(playerId, {
          ...
          isConnected: true,
          accountId: session.accountId ?? void 0
        });
      }
      ...
      if (existingSocket) {
        ...
        existingSocket.close(1e3, "Replaced by new connection");
    ...
    if (parsed.type === "submit_submission") {
      ...
      const categoryMap = this.submissions.get(parsed.categoryId) ?? /* @__PURE__ */ new Map();
  ...
  buildRoundEntries(categoryId) {
    const entries = [];
    let index = 1;
    const categoryMap = categoryId ? this.submissions.get(categoryId) : void 0;
  ...
  resolveRpsWinner(tiebreak) {
    ...
    const [first, second] = nonEmpty.map(([choice]) => choice);
  ...
  concludeRoundWithWinner(winnerId) {
    if (!this.round) return;
    const result = {
  ...
  updateScoreboard(winnerId, categoryId, categoryName) {
  ...
  async recordMatchStats() {
    ...
    const winsByAccount = /* @__PURE__ */ new Map();
  ...
  async hasAudienceEntitlement(accountId) {
...
// .wrangler/tmp/bundle-cf6LXN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cf6LXN/middleware-loader.entry.ts
